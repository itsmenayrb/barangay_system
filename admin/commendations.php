			                ...
			                <table class="table table-hover table-striped">
			                  <tbody>
												...
												<?php 
													...
													$results = $conn -> query("SELECT * FROM commendations");
													...
													while($row = $results -> fetch_assoc()) {
														$id = $row['id'];
														$subject = $row['subject'];
														$message = substr($row['commendationMessage'], 0, 50);

														$template = "
															<tr>
																<td><input type='checkbox'></td>
																<td class='mailbox-star'><a href='#'><i class='fa fa-star text-warning'></i></a></td>
																<td class='mailbox-name'><a href='./message.display.php?commend=$id'>$subject</a></td>
